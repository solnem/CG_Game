...
Camera::Camera(GLFWwindow* pWin) : m_Position(0.f, 7.0f, 31.0f), m_Target(0.f, -8.5f, 13.5f), m_Up(0.0f, 1.0f, 0.0f), m_LastMouseX(-1), m_LastMouseY(-1), m_Panning(0, 0, 0), m_Zoom(0, 0, 0), m_Rotation(0, 0, 0), WindowWidth(1280), WindowHeight(720), pWindow(pWin)
{
	if (pWindow)
	...
	// used for dynamic camera movement based on player position
	this->playerPosition = 0.f;
}
...
void Camera::setPlayerPosition(const float playerPosition)
{
...
void Camera::update()
{
	// disable mouse input for camera control
	updateMouseInput();

	Vector Pos = position(); //m_Position + m_Panning + m_Zoom + m_Rotation;

	// add camera movement according to player position
	Pos.X += (this->playerPosition * 0.75f);
